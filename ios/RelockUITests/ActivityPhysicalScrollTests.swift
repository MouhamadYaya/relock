...
final class ActivityPhysicalScrollTests: XCTestCase {
  ...
  /// Validation opt-in du vrai bouclier Screen Time sur l'iPhone branché.
  /// Ce test n'est jamais exécuté dans la suite standard : Facebook et une
  /// protection Family Controls réelle n'existent pas sur le simulateur.
  @MainActor
  func testRelockSystemShieldOnPhysicalDevice() throws {
    guard ProcessInfo.processInfo.environment["RELOCK_PHYSICAL_SHIELD_TEST"] == "1" else {
      throw XCTSkip("Test réservé à la validation manuelle sur l'iPhone de développement.")
    }

    XCUIDevice.shared.orientation = .portrait
    let protectedApp = XCUIApplication(bundleIdentifier: "com.facebook.Facebook")
    protectedApp.launch()

    // ManagedSettingsUI rend le Shield hors du processus Facebook : son texte
    // n'est pas exposé dans la hiérarchie XCUI de l'app protégée. La capture
    // constitue donc la preuve visuelle, puis le geste reproduit un vrai doigt.
    let shieldSettled = XCTestExpectation(description: "Rendu du Shield système")
    DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
      shieldSettled.fulfill()
    }
    wait(for: [shieldSettled], timeout: 3)
    addScreenshot(protectedApp.screenshot(), name: "bouclier-relock-facebook-iphone")

    protectedApp.coordinate(
      withNormalizedOffset: CGVector(dx: 0.50, dy: 0.85)
    ).tap()

    let relock = XCUIApplication(bundleIdentifier: "com.yaya.relock")
    let continueButton = relock.buttons["breathing-continue"].firstMatch
    XCTAssertTrue(
      continueButton.waitForExistence(timeout: 20),
      "Ouvrir Relock doit lancer la respiration pour l'app exacte, sans déverrouiller.")
    XCTAssertFalse(
      continueButton.isEnabled,
      "La respiration de six secondes doit précéder le choix de durée.")
    addScreenshot(relock.screenshot(), name: "shield-vers-respiration-relock")
  }
