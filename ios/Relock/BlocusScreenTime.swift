...
final class BlocusScreenTime: NSObject {
  ...
  ///
  /// ⚠️ Ne JAMAIS toucher à `days.<id>` ici : cette clé porte le filtre
  /// « lun→ven » d'une plage, et son absence vaut « tous les jours » côté
  /// moniteur. L'effacer transformait silencieusement une plage de semaine en
  /// plage 7 j/7 dès la première pause — d'autant plus sûrement que l'appelant
  /// (`useResumeRuleMutation`, `resumeExpiredSuspensions`) ré-arme la règle
  /// JUSTE AVANT, donc réécrit la clé qu'on supprimait ensuite.
  ...
  func clearRuleData(
    ...
    // supprimée laisse sa clé dans l'App Group pour toujours.
    defaults?.removeObject(forKey: "days.\(ruleId)")
    center.stopMonitoring([DeviceActivityName("resume.\(ruleId)")])
    recomputeShield()
    if kind == "timed", #available(iOS 16.2, *) { endBlockActivities() }
  ...
  #if DEBUG
    /// Fixture visuel de référence, inactif par défaut même sur simulateur.
    /// `-HomeReferenceFixture YES` l'active via le domaine d'arguments de
    /// UserDefaults. Aucun de ces nombres n'existe dans une build Release.
    @objc(homeReferenceFixture:rejecter:)
    func homeReferenceFixture(
      _ resolve: @escaping RCTPromiseResolveBlock,
      rejecter reject: @escaping RCTPromiseRejectBlock
    ) {
      guard UserDefaults.standard.bool(forKey: "HomeReferenceFixture") else {
        resolve(nil)
        return
      }
      resolve([
        "enabled": true,
        "streak": UserDefaults.standard.string(forKey: "HomeMyAppsScenario") == nil ? 13 : 1,
        "focusScore": 78,
        "restScore": 66,
        "blockedCount": 3,
        "blockedOverflow": 2,
        "myAppsScenario": UserDefaults.standard.string(forKey: "HomeMyAppsScenario") ?? "blocked",
      ])
    }

    /// DEBUG uniquement — rejoue l'EFFET exact de `eventDidReachThreshold`
    /// (quota du jour atteint) depuis l'app, sans attendre qu'iOS compte de
    /// vraies minutes d'usage.
    ///
    /// Pourquoi c'est nécessaire : DeviceActivity ne permet à personne de
    /// provoquer un franchissement de seuil à la demande, et un test qui
    /// devrait consommer 60 minutes d'écran réel n'est pas un test. On rejoue
    /// donc les mêmes écritures que le moniteur — marqueur du jour, fenêtre
    /// « limit.<id> » ouverte, recalcul du bouclier — ce qui vérifie
    /// exactement ce que l'utilisateur ressent : le mur tombe pendant qu'il
    /// est DANS l'app. Ce que ça ne prouve pas : que le seuil iOS se déclenche
    /// (ça, seul `limitProgress` rempli par le moniteur l'atteste).
    /// Jamais compilé en Release.
    @objc(simulateLimitReached:reached:resolver:rejecter:)
    func simulateLimitReached(
      ...
      Self.withGroupLock {
        if reached {
          defaults?.set("\(Self.dayKey()):100", forKey: "limitProgress.\(ruleId)")
          defaults?.set(Self.dayKey(), forKey: "limitReached.\(ruleId)")
        } else {
          defaults?.removeObject(forKey: "limitProgress.\(ruleId)")
          defaults?.removeObject(forKey: "limitReached.\(ruleId)")
        }
      ...
      setWindow(raw, active: reached)
      recomputeShield()
      resolve([
        "activeWindows": defaults?.stringArray(forKey: "activeWindows") ?? [],
        "shieldApplications": store.shield.applications?.count ?? 0,
      ])
    }
  #endif
  ...
  /// Score d'Accueil calcule par l'extension `RelockActivityReport` et depose
  /// dans le conteneur App Group. Les mesures de Temps d'ecran ne sortent
  /// jamais du bac a sable Apple : cote app on ne lit que le resultat agrege.
  ///
  /// `status` vaut `pending` tant qu'il n'y a pas assez de jours mesures —
  /// l'appelant doit afficher « calcul en cours », pas un zero.
  @objc(homeScore:rejecter:)
  func homeScore(
    _ resolve: @escaping RCTPromiseResolveBlock,
    rejecter reject: @escaping RCTPromiseRejectBlock
  ) {
    // L'extension ecrit depuis un AUTRE processus : le cache memoire
    // d'UserDefaults cote app peut etre perime et rater sa derniere ecriture.
    defaults?.synchronize()
    guard let payload = defaults?.dictionary(forKey: "homeScore") else {
      resolve(["status": "pending", "historyDays": 0])
      return
    }
    resolve(payload)
  }
  ...
  func pullEvents(
    ...
    defaults?.synchronize()
    let legacyLog = defaults?.array(forKey: "eventLog") as? [[String: Any]] ?? []
    let shieldLog = ShieldAttemptStore.production()?.eventRecords().map(\.dictionary) ?? []
    resolve(legacyLog + shieldLog)
  }
  ...
  func ackEvents(
    ...
    var remaining = max(0, count.intValue)
    Self.withGroupLock {
      var log = defaults?.array(forKey: "eventLog") as? [[String: Any]] ?? []
      let n = min(remaining, log.count)
    ...
    ShieldAttemptStore.production()?.acknowledgeEvents(remaining)
    resolve(true)
  }
  ...
  func resetIfFreshInstall(
    ...
    #if DEBUG
      // ⚠️ En développement, une simple réinstallation Xcode est
      // INDISCERNABLE d'une vraie première installation : le drapeau vit dans
      // le sandbox de l'app, qu'un remplacement de build remet à zéro. La
      // purge ci-dessous effaçait donc tout l'App Group — les sélections
      // d'apps comprises — à CHAQUE build. Résultat : les règles restaient en
      // base mais ne bloquaient plus rien, et il fallait re-choisir ses apps
      // dans le sélecteur système à chaque fois. On ne purge donc qu'en
      // Release, où « installation fraîche » veut dire ce qu'il prétend.
      resolve(false)
      return
    #else
    // Une configuration Release installée directement par Xcode/devicectl
    // contient encore un profil de développement. Comme Xcode remplace le
    // conteneur de l'app, `UserDefaults.standard` repart à zéro alors que
    // l'App Group et les protections utilisateur subsistent : la traiter
    // comme une désinstallation volontaire effacerait les sélections à chaque
    // essai sur iPhone. Une distribution App Store/TestFlight n'embarque pas
    // ce profil et conserve le comportement de remise à zéro attendu.
    ...
    shieldStore?.resetKeepingFreshPendingRequest()
  ...
  /// Consomme une seule fois le signal déposé par RelockShieldAction avant que
  /// le système n'ouvre l'app parentale. Le contexte d'app est facultatif : la
  /// redirection vers Blocages fonctionne aussi pour un mur de catégorie qui
  /// ne livre aucun jeton exact. Le store purge les demandes de plus de 5 min.
  @objc(consumePendingShieldRequest:rejecter:)
  func consumePendingShieldRequest(
    _ resolve: @escaping RCTPromiseResolveBlock,
    rejecter reject: @escaping RCTPromiseRejectBlock
  ) {
    guard
      let request = ShieldAttemptStore.production()?.consumeOpenRequest()
    else {
      resolve(NSNull())
      return
    }
    let payload: [String: Any] = [
      "id": request.id,
      "applicationKey": request.applicationKey ?? NSNull(),
      "applicationName": request.applicationName ?? NSNull(),
  ...
  func getDiagnostics(
    ...
    func when(_ timestamp: TimeInterval?) -> String {
      guard let timestamp, timestamp > 0 else { return "jamais" }
      return iso.string(from: Date(timeIntervalSince1970: timestamp))
    }
    ...
    // VÉRITÉ SYSTÈME du blocage. `blocus.isBlocking` est un drapeau que NOUS
    // écrivons : il dit ce qu'on croit avoir fait. Relire le magasin
    // ManagedSettings dit ce qu'iOS applique réellement — c'est la seule
    // mesure qui distingue « la règle est armée » de « les apps sont bloquées ».
    var shieldApplications = -1
    var shieldWebDomains = -1
    var shieldCategories = "indisponible"
    if #available(iOS 16.0, *) {
      let live = store
      shieldApplications = live.shield.applications?.count ?? 0
      shieldWebDomains = live.shield.webDomains?.count ?? 0
      switch live.shield.applicationCategories {
      case .none: shieldCategories = "aucune"
      case .all(let except): shieldCategories = "toutes sauf \(except.count)"
      case .specific(let set, let except):
        shieldCategories = "\(set.count) (sauf \(except.count))"
      @unknown default: shieldCategories = "?"
      }
    }

    // Filtre de jours par règle : son effacement accidentel transforme une
    // plage « lun→ven » en plage 7 j/7 sans que rien ne le signale à l'écran.
    var ruleDays: [String: [Int]] = [:]
    var suspendedRules: [String: Double] = [:]
    for (key, value) in defaults?.dictionaryRepresentation() ?? [:] {
      if key.hasPrefix("days."), let days = value as? [Int] {
        ruleDays[String(key.dropFirst("days.".count))] = days
      }
      if key.hasPrefix("suspendedUntil."), let until = value as? Double {
        suspendedRules[String(key.dropFirst("suspendedUntil.".count))] = until
      }
    }

    let store = ShieldAttemptStore.production()
    let shieldDiagnostics = store?.diagnostics()
    let shieldLog = store?.eventRecords().map(\.dictionary) ?? []
    let log = legacyLog + shieldLog
    let lastShieldPresentation = store?.lastPresentation()
    let pendingShieldRequest = store?.peekOpenRequest()
    let pendingShieldDiagnostic: Any
    if let pendingShieldRequest {
      pendingShieldDiagnostic = [
        "id": pendingShieldRequest.id,
        "applicationKey": pendingShieldRequest.applicationKey ?? NSNull(),
        "applicationName": pendingShieldRequest.applicationName ?? NSNull(),
        "requestedAt": pendingShieldRequest.requestedAt,
      ] as [String: Any]
    } else {
      pendingShieldDiagnostic = NSNull()
    }
    resolve([
      "nativeBuiltAt": builtAt,
      "authorized": authorized,
      "appGroupOK": defaults != nil && store != nil,
      "shieldStateTransport": "app-group-file-v2",
      "eventLogCount": log.count,
      "eventLogTail": Array(log.suffix(5)),
      "totalResisted": max(
        defaults?.integer(forKey: "totalResisted") ?? 0,
        shieldDiagnostics?.totalResisted ?? 0),
      "activeWindows": defaults?.stringArray(forKey: "activeWindows") ?? [],
      "shieldApplications": shieldApplications,
      "shieldWebDomains": shieldWebDomains,
      "shieldCategories": shieldCategories,
      "reprievedCount": Self.liveReprieves(defaults).count,
      "ruleDays": ruleDays,
      "suspendedRules": suspendedRules,
      "monitorLastWakeAt": when("monitor.lastWakeAt"),
      "shieldShownTotal": max(
        defaults?.integer(forKey: "shieldShownTotal") ?? 0,
        shieldDiagnostics?.shownTotal ?? 0),
      "shieldLastShownAt": shieldDiagnostics?.lastShownAt.map { when($0) }
        ?? when("shield.lastShownAt"),
      // Quelle app a buté sur le mur en dernier. Les jetons étant opaques,
      // c'est la seule façon de relier un affichage de bouclier à une app.
      "shieldProbeAt": shieldDiagnostics?.probeAt.map { when($0) }
        ?? when("shield.probeAt"),
      "shieldProbeWhat": shieldDiagnostics?.probeWhat
        ?? defaults?.string(forKey: "shield.probeWhat") ?? "—",
      "shieldLastApplicationName": lastShieldPresentation?.applicationName ?? "—",
      "shieldLastApplicationCount": lastShieldPresentation?.count ?? 0,
      "armedActivities": {
        if #available(iOS 16.0, *) {
      ...
      "monitorLastWakeWhat": defaults?.string(forKey: "monitor.lastWakeWhat")
        ?? "—",
      "shieldLastActionAt": shieldDiagnostics?.lastActionAt.map { when($0) }
        ?? when("shield.lastActionAt"),
      "shieldLastOpenRequestStatus": shieldDiagnostics?.lastOpenRequestStatus
        ?? defaults?.string(forKey: "shield.lastOpenRequestStatus") ?? "—",
      "shieldLastActionResponse": shieldDiagnostics?.lastActionResponse
        ?? defaults?.string(forKey: "shield.lastActionResponse") ?? "—",
      "pendingShieldRequest": pendingShieldDiagnostic,
    ])
  }
}
